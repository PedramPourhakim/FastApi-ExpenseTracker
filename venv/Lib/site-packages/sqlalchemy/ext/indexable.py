...
Given ``PersonModel`` as a model with a primary key and JSON data field.
...
can compose a new ``PersonModel`` and set the value of ``name``::

    >>> person = PersonModel(name="Alchemist")
...
Below, we illustrate ``PersonModel.name`` used to generate
an indexed SQL criteria::

    >>> from sqlalchemy.orm import Session
    >>> session = Session()
    >>> query = session.query(PersonModel).filter(PersonModel.name == "Alchemist")

The above query is equivalent to::

    >>> query = session.query(PersonModel).filter(PersonModel.data["name"] == "Alchemist")
...
Above, a query such as::
    ...
    q = session.query(PersonModel).filter(PersonModel.year == "1980")
...
for indexed access, instead of the usual index operator of ``->``::
    ...
    >>> query = session.query(PersonModel).filter(PersonModel.age < 20)
